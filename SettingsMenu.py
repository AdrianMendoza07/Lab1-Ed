...
def runSettingsMenu(screen, events, bg):
    ...
    if not hasattr(runSettingsMenu, "initialized") or runSettingsMenu.initialized == False:
        ...
        runSettingsMenu.saveButton = Button(
            ...
            button_width,
            button_height,
            (center_x - button_width - 10, int(HEIGHT * 0.85)),
        ...
        # Dificultad
        runSettingsMenu.easy_rect = pygame.Rect(int(WIDTH * 0.4), int(HEIGHT * 0.5), int(WIDTH * 0.12 * scale), int(HEIGHT * 0.07 * scale))
        runSettingsMenu.hard_rect = pygame.Rect(int(WIDTH * 0.55), int(HEIGHT * 0.5), int(WIDTH * 0.12 * scale), int(HEIGHT * 0.07 * scale))

        # Fullscreen
        runSettingsMenu.fs_on_rect = pygame.Rect(int(WIDTH * 0.4), int(HEIGHT * 0.65), int(WIDTH * 0.12 * scale), int(HEIGHT * 0.07 * scale))
        runSettingsMenu.fs_off_rect = pygame.Rect(int(WIDTH * 0.55), int(HEIGHT * 0.65), int(WIDTH * 0.12 * scale), int(HEIGHT * 0.07 * scale))
    ...
    # Título (más centrado ahora)
    title = runSettingsMenu.title_font.render("Opciones", True, (210, 15, 240))
    screen.blit(title, title.get_rect(center=(WIDTH//2, int(HEIGHT*0.2))))
    ...
    # Dificultad
    screen.blit(font.render("Dificultad:", True, (255,255,255)), (int(WIDTH*0.2), int(HEIGHT*0.5)))
    ...
    # Fullscreen
    screen.blit(font.render("Pantalla:", True, (255,255,255)), (int(WIDTH*0.2), int(HEIGHT*0.65)))
